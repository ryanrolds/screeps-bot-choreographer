...
class PathCache {
  ...
  setCachedPath(originKey, destKey, value, time) {
    let item = new PathCacheItem(originKey, destKey, value, time);
    this.head.add(item);

    // Add path to cache
    let origins = this.originGoalToPathMap[originKey];
    if (!origins) {
  ...
  getCachedPath(originKey, destKey) {
    let destinations = this.originGoalToPathMap[originKey]
    if (!destinations) {
      return null;
    }

    let item = destinations[destKey];
    if (!item) {
      return null
    }
      ...
      if (featureFlags.getFlag(featureFlags.USE_PATH_SEARCH)) {
        const opts = {
          ...
          roomCallback: (roomName) => {
            ...
            let costMatrix = room.getCostMatrix()
            return costMatrix;
          }
        }
