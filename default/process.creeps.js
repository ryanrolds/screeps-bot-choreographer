...
module.exports.spawnSuicide = () => {
    ...
    let minEnergy = _.max([400, maxEnergy * 0.9])
...
function getBodyParts(role, maxEnergy) {
    ...
    while (true) {
        ...
        let estimate = parts.concat([nextPart]).reduce((acc, part) => {
            return acc + BODYPART_COST[nextPart]
        }, 0)

        //console.log("estiamte", estimate, maxEnergy)

        if (estimate < maxEnergy) {
            parts.push(nextPart)
            total = estimate

            console.log("estimate parts", parts, estimate, maxEnergy)
        } else {
            break
        }
    ...
    console.log("using parts", parts, maxEnergy)
